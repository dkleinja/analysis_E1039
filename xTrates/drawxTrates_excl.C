void drawxTrates_excl(const int tarpos = 350, const int magpos = 150)
{
...
GxT_e906 -> SetTitle("Accepted Drell-Yan #mu^{+}#mu^{-} pairs rates");
  ...
  legend -> AddEntry(GxT_p1,"0 Tesla [-300 cm]");
  //legend -> AddEntry(GxT_p2,"+2 Tesla");
  legend -> Draw();
  
  sprintf(Hname, "./images_xTrates/exclusion_dimuon_target_tarpos%d_magpos%d.gif", tarpos, magpos);
  c1 -> SaveAs(Hname);
