...
void drawbeam3(const float xdrift = 0., const float ydrift = 0.)
{
  ...
  for(int i = 0; i < nentries; i++){
    ...
    xvtx_neg = xvtx - xdrift;
    yvtx_neg = yvtx - ydrift;
    
    if(xvtx*xvtx/xswic/xswic + yvtx*yvtx/yswic/yswic < 1){
    //if(xvtx*xvtx/xtarget/xtarget + yvtx*yvtx/ytarget/ytarget < 1){
    ...
    //if(xvtx_neg*xvtx_neg/xtarget/xtarget + yvtx_neg*yvtx_neg/ytarget/ytarget < 1){
    if(xvtx_neg*xvtx_neg/xswic/xswic + yvtx_neg*yvtx_neg/yswic/yswic < 1){
      Hneg -> Fill(xvtx_neg, yvtx_neg);
    }
    //if(xvtx_pos*xvtx_pos/xtarget/xtarget + yvtx_pos*yvtx_pos/ytarget/ytarget < 1){
    if(xvtx_pos*xvtx_pos/xswic/xswic + yvtx_pos*yvtx_pos/yswic/yswic < 1){
    //if((xvtx_pos-xdrift)*(xvtx_pos-xdrift)/(xtarget)/(xtarget) + (yvtx_pos-ydrift)*(yvtx_pos-ydrift)/(ytarget)/(ytarget) > 1){
  ...
  cout << "The error due to beam shift is:  " << fabs(Hbeam -> GetEntries() - Hneg -> GetEntries()) /  Hbeam -> GetEntries() << endl;
