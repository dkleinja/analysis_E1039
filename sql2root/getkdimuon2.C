void getkdimuon2(const char *schema, const int val = 0)
{
  char stmt [1024];
  char delstmt [64];
  char seedstmt [64];
  ...
  sprintf(seedstmt, "USE %s", schema);
   ...
   sprintf(delstmt, "DROP TABLE IF EXISTS kDimuon_%d", val);

   sprintf(stmt, // This first part tells it where to put the data you want
           "CREATE TABLE kDimuon_%d "
           "SELECT kDimuon.* FROM kDimuon "
