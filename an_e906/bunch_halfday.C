...
void bunch_halfday(const int Trig = 1, const int Roadset = 67, const int NBins = 1)
{
  ...
  sprintf(inName, "./results_tighter/asym2_roadset%d_nbins%d_halfday0.root", Roadset, nbins);
  ...
  for (int i = 1; i < NFILES; i++){
    ...
    //load the inFile   
    sprintf(inName, "./results/asym2_roadset%d_nbins%d_halfday%d.root", Roadset, nbins, i);
    sprintf(inName, "./results_tighter/asym2_roadset%d_nbins%d_halfday%d.root", Roadset, nbins, i);
    //printf(inName);
