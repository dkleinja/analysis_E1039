...
void bunch_thirdday(const int Trig = 0, const int Roadset = 67, const int NBins = 1)
{
  ...
  for (int i = 0; i < NFILES; i++){
    ...
    //load the inFile   
    if(Trig == 0) sprintf(inName, "./results/asym2_roadset%d_nbins%d_thirdday%d.root", Roadset, nbins, i);
    //printf(inName);
  ...
  for(int a = 1; a < 2; a++){
    ...
    ca1[a] -> cd();
    sprintf(Hname, "./bunch_an1_thirdday_nbins%d_roadset%d_target%d.gif", nbins, Roadset, a);
    ca1[a] -> SaveAs(Hname);
    ca5[a] -> cd();
    sprintf(Hname, "./bunch_an5_thirdday_nbins%d_roadset%d_target%d.gif", nbins, Roadset, a);
    ca5[a] -> SaveAs(Hname);
  }
