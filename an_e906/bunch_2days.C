...
void bunch_2days(const int Trig = 0, const int Roadset = 67, const int NBins = 1)
{
  ...
  for(int a = 0; a < 2; a++){
    ...
    sprintf(Tname, "#chi^{2}/(NDF-1) for #LT2 day#GT spin change");
	...
	if(NBins == 1){xfTitleMin = 0.2, xfTitleMax = 0.7;}
      ...
      sprintf(Tname, "Sqrt A_{N}/#sigma_{A_{N}} 2 day Shuff. for Roadset %d, %1.1f < x_{F} < %1.1f for %s", Roadset, xfTitleMin, xfTitleMax, inArm);
      ...
      sprintf(Tname, "Pol A_{N} 2 day Shuff. for Roadset%d, %1.1f < x_FT} < %1.1f for %s", Roadset, xfTitleMin, xfTitleMax, inArm);
      ...
      sprintf(Tname, "A_{N} 2 day Shuff. for Roadset %d, %1.1f < x_{F} < %1.1f for %s", Roadset, xfTitleMin, xfTitleMax, inArm);
      sprintf(Tname, "A_{N} for #LT12 day#GT spin change", Roadset, xfTitleMin, xfTitleMax, inArm);
      ...
      sprintf(Tname, "Sqrt #sigma_{A_{N}} 2 day Shuff. for Roadset %d, %1.1f < x_{F} < %1.1f for %s", Roadset, xfTitleMin, xfTitleMax, inArm);
      sprintf(Tname, "#sigma_{A_{N}} for #LT2 day#GT spin change", Roadset, xfTitleMin, xfTitleMax, inArm);
      ...
      sprintf(Hname, "anval5_target%d_xf%d", a, k);
      sprintf(Tname, "Pol A_{N} 2 day Shuff. for Roadset %d, %1.1f < x_{F} < %1.1f for %s", Roadset, xfTitleMin, xfTitleMax, inArm);
  ...
  sprintf(inName, "./results/asym2_roadset%d_nbins%d_2days0.root", Roadset, nbins);
  sprintf(inName, "./results_tighter/asym2_roadset%d_nbins%d_2days0.root", Roadset, nbins);
  ...
  for (int i = 1; i < NFILES; i++){
    ...
    //load the inFile   
    sprintf(inName, "./results/asym2_roadset%d_nbins%d_2days%d.root", Roadset, nbins, i);
    sprintf(inName, "./results_tighter/asym2_roadset%d_nbins%d_2days%d.root", Roadset, nbins, i);
    //printf(inName);
  ...
  for(int a = Trig; a < Trig+1; a++){
    ...
    for (int k = 0; k < nbins; k++){ // These are the slices in xf !!!
      ...
      anval1err[a][k] -> Draw();
      ca1[a] -> cd (3*k + 2);
      //anval1[a][k] -> Fit(gausdist1[a][k],"R");
      anval1[a][k] -> Draw();
      if(NBins == 1){
	ca1[a] -> cd (3*k + 3);
	Hfit_rndf1[a] -> Draw();
    ...
    ca1[a] -> cd();
    sprintf(Hname, "./bunch_an1_2days_nbins%d_roadset%d_target%d.gif", nbins, Roadset, a);
    ca1[a] -> SaveAs(Hname);
    ca5[a] -> cd();
    sprintf(Hname, "./bunch_an5_2days_nbins%d_roadset%d_target%d.gif", nbins, Roadset, a);
    ca5[a] -> SaveAs(Hname);
  }
