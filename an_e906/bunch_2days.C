...
void bunch_2days(const int Trig = 0, const int Roadset = 67, const int NBins = 1)
{
  ...
  for(int a = 0; a < 2; a++){
	...
	if(NBins == 1){xfTitleMin = 0.2, xfTitleMax = 0.7;}
      ...
      sprintf(Tname, "Sqrt A_{N} 2 day Shuff. for Roadset %d, %1.1f < x_{F} < %1.1f for %s", Roadset, xfTitleMin, xfTitleMax, inArm);
  ...
  for (int i = 0; i < NFILES; i++){
    ...
    //load the inFile   
    if(Trig == 0) sprintf(inName, "./results/asym2_roadset%d_nbins%d_2days%d.root", Roadset, nbins, i);
    //printf(inName);
