...
void asym(const int Trig = 0, const int NBins = 2, const int Peak = 0, const int Bkgnd = 0, const int RMIN = 11, const int RMAX = 19, const int Fill = 0, const int Train = 249, const int Final = 1, const int Hot = 1, const int Xing = 1)
{
  ...
  char inBeam[7];
  char inPol[5];
  if(Trig == 0) sprintf (inTrig, "MB");
  else if(Trig == 1) sprintf (inTrig, "4x4b");
  else if(Trig == 2) sprintf (inTrig, "Sim");

  //Beam polarization
  Double_t bluePol = .488;
  Double_t bluePolError = 0.017;
  Double_t yellowPol = 0.418;
  Double_t yellowPolError = 0.014;
  if(Fill == 1){bluePol = 0.505; bluePolError = 0.012; yellowPol = 0.416; yellowPolError = 0.013;}
  else if(Fill == 2){bluePol = 0.472; bluePolError = 0.020; yellowPol = 0.411; yellowPolError = 0.014;}
  else if(Fill == 3){bluePol = 0.495; bluePolError = 0.016; yellowPol = 0.429; yellowPolError = 0.014;}
  else if(Fill == 4){bluePol = 0.491; bluePolError = 0.020; yellowPol = 0.420; yellowPolError = 0.017;}
  char Beam[6];
  char Poldir[5];
  char LRdir[6];
  ...
  Int_t binjump = 3;if(Trig == 1){binjump = 5;}
  Int_t mbins = 140;
  Double_t mmin = 0.;
  Double_t mmax = 1.4;
  ...
  //Yield histograms
  TH2F *Yield[2][nbins][2][2]; // [arm][pt][beam][pol]
  TH2F *Yieldmix[2][nbins];
  TH2F *Yield2[2][nbins][2][2]; // [arm][pt][beam][pol]
  TH2F *Yieldmix2[2][nbins];
  TH2F *Yield3[2][nbins]; // [arm][pt]
  ...
  for (int p = 0; p < phibins; p++)
	...
	for (int k = 0; k < nbins; k++){
	  ...
	  drawstart = 0.25;
	  sprintf(Hname, "drawboth_pt%d_%d_%d_%d", k, p, i, j);
  ...
  for (int p = 0; p < 2; p++)
	...
	for (int k = 0; k < nbins; k++){
	  drawstart = 0.25 + float(Trig) * 0.05 + float(Trig) * float(k) * 0.05;
	  //drawstart = 0.25;
	  sprintf(Hname, "drawboth2_pt%d_%d_%d_%d", k, p, i, j);
  ...
  for (int a = 0; a < 2; a++)
    for (int k = 0; k < nbins; k++){
      //drawstart = 0.25 + float(Trig) * 0.05 + float(Trig) * float(k) * 0.05; 
      drawstart = 0.25;
      sprintf(Hname, "drawboth3_arm%d_pt%d", a, k);
 ...
 Float_t ratio_min2 = 0.3;
  ...
  TH2F *yptmass  = new TH2F ("yptmass",  "yptmass",  100, 0., 1., 500, 0., 5. );
  TH2F *yptmass3  = new TH2F ("yptmass3",  "yptmass3",  100, 0., 1., 500, 0., 5. );
  TH1F *yeta  = new TH1F ("yeta",  "yeta", 300, 2.0, 5.0 );
  TH3F *yptmassxf  = new TH3F ("yptmassxf",  "yptmassxf",  100, 0., 1., 500, 0., 5., 100, 0., 1. );
  TH3F *yptmassxf3  = new TH3F ("yptmassxf3",  "yptmassxf3",  100, 0., 1., 500, 0., 5., 100, 0., 1. );
  yptmass  -> GetXaxis() -> SetTitle ("mass");
  yptmass  -> GetYaxis() -> SetTitle ("pt");
  yptmassxf  -> GetXaxis() -> SetTitle ("mass");
  yptmassxf  -> GetYaxis() -> SetTitle ("pt");
  yptmass3  -> GetXaxis() -> SetTitle ("mass");
  yptmass3  -> GetYaxis() -> SetTitle ("pt");
  yptmassxf3  -> GetXaxis() -> SetTitle ("mass");
  yptmassxf3  -> GetYaxis() -> SetTitle ("pt");
  ...
  for (int a = 0; a < 2; a++)  
    for (int k = 0; k < nbins; k++){
      sprintf (Hname, "yphi3_arm%d_pt%d", a, k);
	...
	for (int d = 0; d < 2; d++) { // down-up
	  sprintf (Hname, "yphi_arm%d_pt%d_%d_%d", a, k, b, d);
	  yphi[a][k][b][d] = new TH1D (Hname, Hname, phibins, phimin, phimax);
	  yphi[a][k][b][d] -> SetLineColor (d + 1);
	  yphi[a][k][b][d] -> Sumw2();
	  sprintf (Hname, "yphi2_arm%d_pt%d_%d_%d", a, k, b, d);
  ...
  // ---------------------------------------------------------------------
  // Polarization asymmetries [arm][pt][beam]
  TH1D *asymP[2][nbins][2];
  TH1D *asymP2[2][nbins][2];
  for (int a = 0; a < 2; a++) 
    for (int k = 0; k < nbins; k++) {
      sprintf (Hname, "asymPol_blue_arm%d_pt%d", a, k);
      ...
      sprintf (Hname, "asymPol_yellow_arm%d_pt%d", a, k);
  ...
  for (int a = 0; a < 2; a++)  
    for (int k = 0; k < nbins; k++) {
      sprintf (Hname, "asymPol2_blue_arm%d_pt%d", a, k);
      ...
      sprintf (Hname, "asymPol2_yellow_arm%d_pt%d", a, k);
  ...
  TH1D *asymL[2][nbins][2];  // [pt][beam]
  for (int a = 0; a < 2; a++)  
    for (int k = 0; k < nbins; k++) {  
      sprintf (Hname, "asymSide_blue_arm%d_pt%d", a, k);
      ...
      sprintf (Hname, "asymSide_yellow_arm%d_pt%d", a, k);
  ...
  TH1D *asymS[2][nbins][2];  // [pt][beam]
  TH1D *asymS2[2][nbins][2];  // [pt][beam]
  for (int a = 0; a < 2; a++)  
    for (int k = 0; k < nbins; k++) { 
      sprintf (Hname, "asymSqrt_blue_arm%d_pt%d", a, k);
      ...
      sprintf (Hname, "asymSqrt_yellow_arm%d_pt%d", a, k);
      ...
      sprintf (Hname, "asymSqrt2_blue_arm%d_pt%d", a, k);
      ...
      sprintf (Hname, "asymSqrt2_yellow_arm%d_pt%d", a, k);
  ...
  // Asymmetries as function of pt !!!
  TH1F *pt_asymfit1[2][2];
  TH1F *pt_asymfit2[2][2];
  TH1F *pt_phi0fit2[2][2];
  TH1F *pt_rellfit3[2][2];
  TH1F *pt_asymfit3[2][2];
  TH1F *pt_rellfit4[2][2];
  TH1F *pt_asymfit4[2][2];
  TH1F *pt_phi0fit4[2][2];
  TH1F *pt_asymfit5[2][2];
  ...
  TH1F *pt_mean = new TH1F("pt_mean","pt_mean", 10, 0., 5.);
  TH1F *xf_mean = new TH1F("xf_mean","xf_mean", 10, 0., 5.);
  int gstyle;
  for (int a = 0; a < 2; a++)
    for (int b = 0; b < 2; b++){
      if(a == 0 && b == 0){gstyle = 25;}
      else if(a == 0 && b == 1){gstyle = 21;}
      else if(a == 1 && b == 0){gstyle = 21;}
      else if(a == 1 && b == 1){gstyle = 25;}
      
      sprintf(Hname, "pt_asymfit1_arm%d_xf%d", a, b);
      pt_asymfit1[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
      pt_asymfit1[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
      pt_asymfit1[a][b] -> GetYaxis() -> SetTitle ("raw asymmetry #epsilon");
      SetHistoStyle (pt_asymfit1[a][b], gstyle, 2, 1); 

      sprintf(Hname, "pt_asymfit2_arm%d_xf%d", a, b);
      pt_asymfit2[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
      sprintf(Hname, "pt_phi0fit2_arm%d_xf%d", a, b);
      pt_phi0fit2[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
      pt_asymfit2[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
      pt_asymfit2[a][b] -> GetYaxis() -> SetTitle ("raw asymmetry #epsilon");
      pt_phi0fit2[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
      pt_phi0fit2[a][b] -> GetYaxis() -> SetTitle ("#varphi_{0}");
      SetHistoStyle (pt_asymfit2[a][b], 24, 2, 1);
      SetHistoStyle (pt_phi0fit2[a][b], 24, 2, 1);
      SetHistoAxes  (pt_phi0fit2[a][b], -1, 1, -.5 * PI, 1.5 * PI);


      sprintf(Hname, "pt_rellfit3_arm%d_xf%d", a, b);
      pt_rellfit3[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
      sprintf(Hname, "pt_asymfit3_arm%d_xf%d", a, b);
      pt_asymfit3[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
      pt_rellfit3[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
      pt_rellfit3[a][b] -> GetYaxis() -> SetTitle ("rel.lumi.");
      pt_asymfit3[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
      pt_asymfit3[a][b] -> GetYaxis() -> SetTitle ("raw asymmetry #epsilon");
      SetHistoStyle (pt_rellfit3[a][b], gstyle, 8,  1); 
      SetHistoStyle (pt_asymfit3[a][b], gstyle, 8,  1);

      sprintf(Hname, "pt_rellfit4_arm%d_xf%d", a, b);  
    pt_rellfit4[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    sprintf(Hname, "pt_asymfit4_arm%d_xf%d", a, b);
    pt_asymfit4[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    sprintf(Hname, "pt_phi0fit4_arm%d_xf%d", a, b);
    pt_phi0fit4[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    pt_rellfit4[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_rellfit4[a][b] -> GetYaxis() -> SetTitle ("rel.lumi.");
    pt_asymfit4[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_asymfit4[a][b] -> GetYaxis() -> SetTitle ("raw asymmetry #epsilon");
    pt_phi0fit4[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_phi0fit4[a][b] -> GetYaxis() -> SetTitle ("#varphi_{0}");
    SetHistoStyle (pt_rellfit4[a][b], 25, 8, 1);
    SetHistoStyle (pt_asymfit4[a][b], 25, 8, 1);
    SetHistoStyle (pt_phi0fit4[a][b], 24, 8, 1);
    SetHistoAxes  (pt_phi0fit4[a][b], -1, 1, -.5 * PI, 1.5 * PI);


    // Yes-yes, this is the final fit: subtract the global rel. luminosity and fit plain cosine !!!
    sprintf(Hname, "pt_asymfit5_arm%d_xf%d", a, b);  
    pt_asymfit5[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    pt_asymfit5[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_asymfit5[a][b] -> GetYaxis() -> SetTitle ("raw asymmetry #epsilon");
    SetHistoStyle (pt_asymfit5[a][b], gstyle, 4, 1);
      
    sprintf(Hname, "fit_rndf1_arm%d_xf%d", a, b);  
    fit_rndf1[a][b] = new TH1F (Hname, Hname, 100, 0., 10.);
    sprintf(Hname, "fit_rndf2_arm%d_xf%d", a, b);  
    fit_rndf2[a][b] = new TH1F (Hname, Hname, 100, 0., 10.);
    sprintf(Hname, "fit_rndf3_arm%d_xf%d", a, b);  
    fit_rndf3[a][b] = new TH1F (Hname, Hname, 100, 0., 10.);
    sprintf(Hname, "fit_rndf4_arm%d_xf%d", a, b);  
    fit_rndf4[a][b] = new TH1F (Hname, Hname, 100, 0., 10.);
    sprintf(Hname, "fit_rndf5_arm%d_xf%d", a, b);  
    fit_rndf5[a][b] = new TH1F (Hname, Hname, 100, 0., 10.);
    fit_rndf1[a][b] -> GetXaxis() -> SetTitle ("#chi^{2} / (n.d.f. - 1)");
    fit_rndf2[a][b] -> GetXaxis() -> SetTitle ("#chi^{2} / (n.d.f. - 1)");
    fit_rndf3[a][b] -> GetXaxis() -> SetTitle ("#chi^{2} / (n.d.f. - 1)");
    fit_rndf4[a][b] -> GetXaxis() -> SetTitle ("#chi^{2} / (n.d.f. - 1)");
    fit_rndf5[a][b] -> GetXaxis() -> SetTitle ("#chi^{2} / (n.d.f. - 1)");

    // And look: then there is the analyzing power !!!
    sprintf(Hname, "pt_an5_arm%d_xf%d", a, b);  
    pt_an5[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    pt_an5[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_an5[a][b] -> GetYaxis() -> SetTitle ("A_{N}");
    SetHistoStyle (pt_an5[a][b], gstyle, 4, 1);
    sprintf(Hname, "pt_an1_arm%d_xf%d", a, b);  
    pt_an1[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    pt_an1[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_an1[a][b] -> GetYaxis() -> SetTitle ("A_{N}");
    SetHistoStyle (pt_an1[a][b], gstyle, 2, 1);
    sprintf(Hname, "pt_an3_arm%d_xf%d", a, b);  
    pt_an3[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    pt_an3[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_an3[a][b] -> GetYaxis() -> SetTitle ("A_{N}");
    SetHistoStyle (pt_an3[a][b], gstyle, 8, 1);
    sprintf(Hname, "pt_an6_arm%d_xf%d", a, b);  
    pt_an6[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    pt_an6[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_an6[a][b] -> GetYaxis() -> SetTitle ("A_{N}");
    SetHistoStyle (pt_an6[a][b], gstyle, 2, 1);
    sprintf(Hname, "pt_an7_arm%d_xf%d", a, b);  
    pt_an7[a][b] = new TH1F (Hname, Hname, 10, 0, 5);
    pt_an7[a][b] -> GetXaxis() -> SetTitle ("p_{T}");
    pt_an7[a][b] -> GetYaxis() -> SetTitle ("A_{N}");
    SetHistoStyle (pt_an7[a][b], gstyle, 4, 1);
  }
  ...
  if(Trig ==0)sprintf (inName, "./nDST/Analysis_roadset%d_R005_V001.root", Train, Fill);
  else if(Trig == 1)sprintf (inName, "./nDST/Analysis_roadset%d_R005_V001.root", Train, Fill);
  ...
  for (int i = 0; i < nentries; i++) {
    ...
    eta = fabs(eta);
    //if(arm != Arm){continue;}
    if(rxy_i < RMIN || rxy_j < RMIN){continue;}
    if(rxy_i > RMAX || rxy_j > RMAX){continue;}
    ...
    if(Trig == 1 && Hot == 1){
      hot_i = 0; hot_j = 0;
      for(int iwarn = 0; iwarn < nwarns; iwarn++){
	if(arm == w_arm[iwarn] && xpos_i == w_xpos[iwarn] && ypos_i == w_ypos[iwarn]){
	  hot_i = 1;
	  //cout << i << " hot_i is hot " << arm << " " << xpos_i << " " << ypos_i << endl;            
	  break;
	}
      }
      if(hot_i == 1 ){continue;}
      for(int iwarn = 0; iwarn < nwarns; iwarn++){
	if(arm == w_arm[iwarn] && xpos_j == w_xpos[iwarn] && ypos_j == w_ypos[iwarn]){
	  hot_j = 1;
	  //cout << i << " hot_j is hot " << arm << " " << xpos_j << " " << ypos_j << endl;
	  break;
	}
      }
      if(hot_j == 1 ){continue;}
    ...
    //xing = cross % 2;   
    //if(xing != Xing){continue;}
    //cout << "Event " << i << ".  Crossing " << cross << " Even/Odd value is " << xing << ".  0 = even and 1 = odd." << endl;
    //let's make a blue, yellow phi.
    //phiyellow = phi;
    phiyellow = phi - PI;
    if(phiyellow < -PI){
    ...
    phiblue = phi - 0.24;
    if(phiblue < -PI){
      phiblue = phiblue + 2*PI;
    }
    /*
    //let's shift the phenix phi south.
    if(arm == 0){
      phiyellow = phiyellow + PI;
      if(phiyellow > PI){
	phiyellow = phiyellow - 2*PI;
      }
      phiblue = phiblue + PI;
    ...
    */
    ...
    if(Trig == 2 && energy > 50){continue;}
    if(Trig == 0 || Trig == 2 || Trig == 3){if(pt < 1.0 || pt > 2.0){continue;}}
    if(Trig == 1){if(pt < 2.0){continue;}}

    if(nbins == 1){k = 0;}
    else{
      if(Trig == 0 || Trig == 2){int k = int (2 * pt - 2);}
      if(Trig == 1){int k = int (2 * pt - 4);}
    }

    if (k < 0) continue;
    if (k > nbins-1) k = nbins - 1;
    //if (k > nbins-1) continue;
    //cout << i << " " << k << " " << pt <<  " " << rxy_i << " " << rxy_j << endl;
    ...
    //get the same event spincombo
     if(Trig == 2){
      ...
      if(i%100000 == 0)  cout << "entry, polB, polY " << i << " " << polB << " " << polY << endl;
       ...
       if(spincombo == 0){polB = 1; polY = 1;}
       if(spincombo == 1){polB = 1; polY = 0;}
       if(spincombo == 2){polB = 0; polY = 1;}
       if(spincombo == 3){polB = 0; polY = 0;}
     }
    ...
    yptmass  -> Fill (mass,pt);
    yeta  -> Fill (eta);
    yptmassxf  -> Fill (mass,pt,xf);
 ...
 // =========================================== Read the mixed events !!! 
  if(Bkgnd == 1){ 
    if(Trig == 0)  sprintf (inName, "./atrain%d/mbmb/fill_%d.root", Train, Fill);
    if(Trig == 1)  sprintf (inName, "./atrain%d/trig/fill_%d.root", Train, Fill);
    if(Trig == 2)  sprintf (inName, "./atrain%d/sim/fill_%d.root", Train, Fill);
    TFile *inFile2 = new TFile (inName);
    gROOT -> cd(); 
    TTree *MixedTree = (TTree*) inFile2 -> Get ("Mix");
    MixedTree -> SetBranchAddress ("arm",   &arm);
    MixedTree -> SetBranchAddress ("spincombo",   &spincombo);
    MixedTree -> SetBranchAddress ("xpos_i", &xpos_i);
    MixedTree -> SetBranchAddress ("ypos_i", &ypos_i);
    MixedTree -> SetBranchAddress ("xpos_j", &xpos_j);
    MixedTree -> SetBranchAddress ("ypos_j", &ypos_j);
    MixedTree -> SetBranchAddress ("rxy_i", &rxy_i);
    MixedTree -> SetBranchAddress ("rxy_j", &rxy_j);
    MixedTree -> SetBranchAddress ("e_pair",   &energy);
    MixedTree -> SetBranchAddress ("easym",   &easym);
    MixedTree -> SetBranchAddress ("eta",   &eta);
    MixedTree -> SetBranchAddress ("mass",  &mass);
    MixedTree -> SetBranchAddress ("phi",   &phi);
    MixedTree -> SetBranchAddress ("pt",    &pt);
    MixedTree -> SetBranchAddress ("xf",    &xf);
    
    nentries = MixedTree -> GetEntries();
    if(nentries > 5000000){nentries = 5000000;}
    cout << "Mixed entries: " << nentries << endl;
    for (int i = 0; i < nentries; i++) {
	...
	if(Trig == 1){int k = int (2 * pt - 4);}
      ...
      //get the mixed event spincombo
      //if(spincombo == 0){polB = 1; polY = 1;}
      //if(spincombo == 1){polB = 1; polY = 0;}
      //if(spincombo == 2){polB = 0; polY = 1;}
      //if(spincombo == 3){polB = 0; polY = 0;}

      Yieldmix[arm][k] -> Fill (mass, phi);
      Yieldmix2[arm][k]-> Fill (mass, phi);
      Yieldmix3[arm][k]-> Fill (mass, phi3);
    }
  ...
  for (int a = 0; a < 2; a++){ // These are the slices in Arms !!
    ...
    for (int k = 0; k < nbins; k++) { // These are the slices in pt !!!
   ...
   //labels!
      float ptTitleMin, ptTitleMax;
      float phiTitleMin, phiTitleMax;
      if(Trig == 0 || Trig == 2 || Trig == 3){
        ptTitleMin = 0.5 * float (k) + 1.0;
        ptTitleMax = 0.5 * float (k) + 1.5;
        if(NBins == 1){ptTitleMin = 1.0, ptTitleMax = 2.0;}
      }
      if( Trig == 1){
        ptTitleMin = 0.5 * float (k) + 2.0;
        ptTitleMax = 0.5 * float (k) + 2.5;
        if(NBins == 1){ptTitleMin = 2.0, ptTitleMax = 4.0;}
      }
      
      //fit range!
      printf("//////////////////////////////////");
      printf("GET PARAMETERS FOR PT BIN %d \n", k);
      ///////////////////////////////////////////////////////////
      ///////////////////////////////////////////////////////////
      double pi0peak;
      double pi0width;
      ...
      c1->cd();
      printf("Doing mass distributions for %s pt bin%d, and beams B%dY%d \n", inArm, a, k);
      if(Bkgnd == 0){
	...
	hmass_sub3 -> Add (hmass3, hmix_norm3, 1., -1.);
      ...
      sprintf (Hname, "%s %s for %1.1f < p_{T} < %1.1f", inArm, inTrig, ptTitleMin, ptTitleMax);
      ...
      if(Peak == 1){
        //pi0_right = pi0_left - 2;
        pi0_right = 45;
        pi0_left = 30;
        //pi0_left = 20 + 5 * Trig + 5 * Trig * k;
        //pi0peak = float(pi0_center)/100;
      }
      //printf("The mass bins to project in phi for pt bin %d are:  center %d, left %d, right %d \n", k, pi0_center, pi0_left, pi0_right);
      if(Peak == 2){
      ...
      printf("The mass bins to project for %s ptbin %d are:  center %d, left %d, right %d \n", inArm, k,  pi0_center, pi0_left, pi0_right);
      if(Peak == 0){
	...
	TLine *l0 = new TLine (mmin, 0, mmax, 0);
      ...
      drawgaus3 -> Draw("same");
      sprintf (Hname, "./images_pt_rxy%d%d/mass3_peak%d_trig%d_nbins%d_arm%d_pt%d.gif", RMIN, RMAX, Peak, Trig, nbins, a, k);
      if(Final == 1)sprintf (Hname, "./images_pt/mass3_peak%d_trig%d_nbins%d_arm%d_pt%d.gif", Peak, Trig, nbins, a, k);
      c1-> SaveAs(Hname);
      ...
      printf("//////////////////////////////////");
      printf("BIG PHI BIN PART FOR PT BIN %d \n", k);
      ///////////////////////////////////////////////////////////
      ///////////////////////////////////////////////////////////
	...
	for (int j = 0; j < 2; j++)
	  for(int p = 0; p < 2; p++){
	    ...
	    c1->cd();
	    //Get invariant mass
	    printf("Doing mass distributions for %s pt bin%d, phi bin %s, %s beam, %s direction \n", inArm, k, LRdir, inBeam, inPol);
	    if(Bkgnd == 0){
	      //tmass2 -> Reset();
	    ...
	    cpb2[i][j] -> cd(p+1);
	    sprintf (Hname, "%s %s %s %s for %1.1f < p_{T} < %1.1f, %s", inArm, inTrig, inBeam, inPol, ptTitleMin, ptTitleMax, LRdir);
	    ...
	    printf ("============================\n");
	    printf ("Eta mass fit: %f (%f) \n", pi0peak, pi0width);
	    printf("The BIG PHI BIN mass bins to project for %s ptbin %d, phibin %d, %s beam, %s direction are:  center %d, left %d, right %d \n", inArm, k, p, inBeam, inPol, pi0_center, pi0_left, pi0_right);
	    ...
	    sprintf (Hname, "%d COUNTS! %s %s %s %s for %1.1f < p_{T} < %1.1f, %s", integral, inArm, inTrig, inBeam, inPol, ptTitleMin, ptTitleMax, LRdir);
	...
	for (int i = 0; i < 2; i++)
	  for (int j = 0; j < 2; j++){
	  cpb2[i][j] -> cd();
          sprintf (Hname, "./images_mass_pt/mass2_nbins%d_trig%d_arm%d_pt%d_B%d_D%d.gif", nbins, Trig, a, k, i, j);
	  cpb2[i][j] -> SaveAs(Hname);
	  //cpa2[i][j] -> cd();
	  //sprintf (Hname, "./images_mass_pt/mass2sub_nbins%d_trig%d_arm%d_pt%d_B%d_D%d.gif", nbins, Trig, a, k, i, j);
	  //cpa2[i][j] -> SaveAs(Hname);
	  }
      }
      printf("//////////////////////////////////");
      printf("LITTLE PHI BIN PART FOR PT BIN %d \n", k);
      //one phi bin at a time, then subtract background!!
      for(int i = 0; i < 2; i++)
	for (int j = 0; j < 2; j++)
	  for(int p = 0; p < phibins; p++){
	    ...
	    if(Bkgnd == 0){
	      ...
	      //Subtract off mixed events
	      printf("Doing mass distributions for %s pt bin%d, phi bin %d,  %s beam, %s direction \n", inArm, k, p, inBeam, inPol); 
	      tmass -> Reset();
	      tmass = Yield[a][k][i][j] -> ProjectionX("tmass", p+1, p+1);
	      hmass[p][i][j] -> Reset();
	      hmass[p][i][j] -> Add (tmass);
	    ...
	    //Proceed with bg subtraction!
	    cpb[i][j] -> cd(p+1);
	    sprintf (Hname, "%s %s %s %s for %1.1f < p_{T} < %1.1f, %1.2f < #phi < %1.2f", inArm, inTrig, inBeam, inPol, ptTitleMin, ptTitleMax, phiTitleMin, phiTitleMax);
	    ...
	    printf ("============================\n");
	    printf ("Eta mass fit: %f (%f) \n", pi0peak, pi0width);
	    printf("The LITTLE PHI BIN mass bins to project for %s ptbin %d, phibin %d, %s beam, %s direction are:  center %d, left %d, right %d \n", inArm, k, p, inBeam, inPol, pi0_center, pi0_left, pi0_right);
...
if(Peak == 0){
	...
	for (int j = 0; j < 2; j++){
	  //cpb[i][j] -> cd();
          //sprintf (Hname, "./images_mass_pt/mass_nbins%d_trig%d_arm%d_pt%d_B%d_D%d.gif", nbins, Trig, a, k, i, j);
	  //cpb[i][j] -> SaveAs(Hname);
	  //cpa[i][j] -> cd();
	  //sprintf (Hname, "./images_mass_pt/masssub_nbins%d_trig%d_arm%d_pt%d_B%d_D%d.gif", nbins, Trig, a, k, i, j);
	  //cpa[i][j] -> SaveAs(Hname);
	}
}
      //Get the #eta meson Yields in phi!!
      printf("NOW GETTING THE SPIN, PHI DEPENDENT YIELDS FOR %s pt bin %d \n", inArm, k);
	...
	for (int p = 0; p < phibins / 2; p++) {
      ...
      double cosavg_num = 0;
      double cosavg = 0;
      printf ("The f correction factor variables for %s pt bin %d. \n", inArm, k);
	...
	printf ("%5d : %1.2f : %5d : %1.2f : %1.1f \n", p, cosval[p], n1, cosavg_num, cosavg_den);
      ...
      printf("The correction factor f for %s pt bin %d is %1.2f. \n", inArm, k, 1/cosavg);
      //end of cos avg function f
      
      /////////////////////////////////////////////////
      //this is new let's just do sqrt, left-right
      printf ("The sqrt-formula spin dependent yields for %s pt bin %d. \n", inArm, k);
      for (int b = 0; b < 2; b++) {     // blue-yellow
	n1 = n2 = n3 = n4 = 0;
	...
	if ((n1 > 9) && (n2 > 9) && (n3 > 9) && (n4 > 9)) {
      ...
      printf ("The relative luminosity spin dependent yields for %s pt bin %d. \n", inArm, k);
      for (int b = 0; b < 2; b++) {        // blue-yellow      
	n1 = n2 = n3 = n4 = 0;
	n1 = yphi2[a][k][b][1] -> GetBinContent (1);
	n2 = yphi2[a][k][b][1] -> GetBinContent (2);
	...
	if ((n1 > 9) && (n2 > 9) && (n3 > 9) && (n4 > 9)) {
      ...
      // Determine the amplitude of the single spin asymmetries as function of pt !!!
      for (int b = 0; b < 2; b++) {
	printf ("==============================================\n");
	printf (" fit results for %s pt-bin %d, beam %d \n", inArm, k, b);
	printf ("----------------------------------------------\n");
	printf ("::: sqrt,pol-asymmetries, left-right ::: \n");
	amp_val = asymS2[a][k][b] -> GetBinContent(1);
	amp_err = asymS2[a][k][b] -> GetBinError(1);
	if(b == 0){
	  pt_an6[a][b] -> SetBinContent (k + binjump, amp_val / bluePol);
	  pt_an6[a][b] -> SetBinError   (k + binjump, amp_err / bluePol);
	}
	else{
	  pt_an6[a][b] -> SetBinContent (k + binjump, amp_val / yellowPol);
	  pt_an6[a][b] -> SetBinError   (k + binjump, amp_err / yellowPol);
	}
	amp_val = asymP2[a][k][b] -> GetBinContent(1);
	amp_err = asymP2[a][k][b] -> GetBinError(1);
	if(b == 0){
	  pt_an7[a][b] -> SetBinContent (k + binjump, amp_val / bluePol);
	  pt_an7[a][b] -> SetBinError   (k + binjump, amp_err / bluePol);
	}
	else{
	  pt_an7[a][b] -> SetBinContent (k + binjump, amp_val / yellowPol);
	  pt_an7[a][b] -> SetBinError   (k + binjump, amp_err / yellowPol);
	}
	...
	pt_asymfit1[a][b] -> SetBinContent (k + binjump, amp_val);
	pt_asymfit1[a][b] -> SetBinError   (k + binjump, amp_err);
	printf (" %5.3f +/- %5.3f \n", amp_val, amp_err);
	ndf     = fitcos -> GetNDF();
	csq     = fitcos -> GetChisquare();
	if (ndf > 1){
	  fit_rndf1[a][b] -> Fill (csq / (ndf - 1));
	}
	if(b == 0){
	  pt_an1[a][b] -> SetBinContent (k + binjump, amp_val / bluePol);
	  pt_an1[a][b] -> SetBinError   (k + binjump, amp_err / bluePol);
	}
	else{
	  pt_an1[a][b] -> SetBinContent (k + binjump, amp_val / yellowPol);
	  pt_an1[a][b] -> SetBinError   (k + binjump, amp_err / yellowPol);
	}
	...
	pt_asymfit2[a][b] -> SetBinContent (k + binjump, amp_val);
	pt_asymfit2[a][b] -> SetBinError   (k + binjump, amp_err);
	pt_phi0fit2[a][b] -> SetBinContent (k + binjump, phi_val - 0.5 * PI);
	pt_phi0fit2[a][b] -> SetBinError   (k + binjump, phi_err);
	printf (" %5.3f +/- %5.3f \n", amp_val, amp_err);
	printf (" %5.3f +/- %5.3f \n", phi_val, phi_err);
	ndf     = fitsin -> GetNDF();
	csq     = fitsin -> GetChisquare();
	if (ndf > 1){
	  fit_rndf2[a][b] -> Fill (csq / (ndf - 1));
	}
	printf ("----------------------------------------------\n");
	...
	pt_rellfit3[a][b] -> SetBinContent (k + binjump, rel_val);
	pt_rellfit3[a][b] -> SetBinError   (k + binjump, rel_err);
	pt_asymfit3[a][b] -> SetBinContent (k + binjump, amp_val);
	pt_asymfit3[a][b] -> SetBinError   (k + binjump, amp_err);
	printf (" %5.3f +/- %5.3f \n", rel_val, rel_err);
	printf (" %5.3f +/- %5.3f \n", amp_val, amp_err);
	ndf     = fitcosP -> GetNDF();
	csq     = fitcosP -> GetChisquare();
	if (ndf > 1){
	  fit_rndf3[a][b] -> Fill (csq / (ndf - 1));
	}
	...
	if (sin (phi_val) < 0.) {phi_val += PI; amp_val *= -1.;}
	phi_val = phase_modulo (phi_val);
	pt_rellfit4[a][b] -> SetBinContent (k + binjump, rel_val);
	pt_rellfit4[a][b] -> SetBinError   (k + binjump, rel_err);
	pt_asymfit4[a][b] -> SetBinContent (k + binjump, amp_val);
	pt_asymfit4[a][b] -> SetBinError   (k + binjump, amp_err);
	pt_phi0fit4[a][b] -> SetBinContent (k + binjump, phi_val - 0.5 * PI);
	pt_phi0fit4[a][b] -> SetBinError   (k + binjump, phi_err);
	printf (" %5.3f +/- %5.3f \n", rel_val, rel_err);
	printf (" %5.3f +/- %5.3f \n", amp_val, amp_err);
	printf (" %5.3f +/- %5.3f \n", phi_val, phi_err);
	ndf     = fitsinP -> GetNDF();
	...
	if (ndf > 1){
	  fit_rndf4[a][b] -> Fill (csq / (ndf - 1));
	}
      }
    }// End of the slices in k !!!
    cout << "END OF SLICES IN PT BINS!" << endl;
    ...
    int negr = 0;
    int posr = 1;
    if(a == 1){negr = 1; posr = 0;}
    TF1 *relBlue   = new TF1 ("relBlue",   "pol0", 0, 5);
    TF1 *relYellow = new TF1 ("relYellow", "pol0", 0, 5);
    pt_rellfit3[a][negr] -> Fit ("relBlue",   "R");
    avRL[0] = relBlue   -> GetParameter (0);
    erRL[0] = relBlue   -> GetParError  (0);
    pt_rellfit3[a][posr] -> Fit ("relYellow", "R");
    ...
    RLine[0] = new TLine (0, avRL[0],  5, avRL[0]); RLine[0] -> SetLineWidth (3);
    RLine[1] = new TLine (0., avRL[1], 5, avRL[1]); RLine[1] -> SetLineWidth (3);
	...
	pt_asymfit5[a][b] -> SetBinContent (k + binjump, amp_val);
	pt_asymfit5[a][b] -> SetBinError   (k + binjump, amp_err);
	printf (" %5.3f +/- %5.3f \n", rel_val, rel_err);
	printf (" %5.3f +/- %5.3f \n", amp_val, amp_err);
	ndf     = fitcosF -> GetNDF();
	csq     = fitcosF -> GetChisquare();
	if (ndf > 1){
	  fit_rndf5[a][b] -> Fill (csq / (ndf - 1));
	}
      
	if(b == 0){
	  pt_an5[a][b] -> SetBinContent (k + binjump, amp_val / bluePol);
	  pt_an5[a][b] -> SetBinError   (k + binjump, amp_err / bluePol);
	}
	else{
	  pt_an5[a][b] -> SetBinContent (k + binjump, amp_val / yellowPol);
	  pt_an5[a][b] -> SetBinError   (k + binjump, amp_err / yellowPol);
	}
      }

    // Images !!!
    if(Peak == 0){
	pt_asymfit1[a][0] -> SetMinimum (-.08);
	pt_asymfit1[a][0] -> SetMaximum ( .13);
	pt_an5[a][0] -> SetMinimum (-.20);
	pt_an5[a][0] -> SetMaximum ( .25);
      }
      else if(Peak == 1 || Peak == 2){
	pt_asymfit1[a][0] -> SetMinimum (-.17);
	pt_asymfit1[a][0] -> SetMaximum ( .17);
	pt_an5[a][0] -> SetMinimum (-.35); 
	pt_an5[a][0] -> SetMaximum ( .25);
      }
    pt_an1[a][0] -> SetMinimum (-.20);
    pt_an1[a][0] -> SetMaximum ( .25);
    pt_an1[a][1] -> SetMinimum (-.20);
    pt_an1[a][1] -> SetMaximum ( .25);
 
    TLatex *Lpeak;
    if(Peak == 0){Lpeak = new TLatex(1.0, 0.2, "#eta meson mass region");}
    else if(Peak == 1){Lpeak = new TLatex(1.0, 0.2, "0.30 < M_{clcl} < 0.45 GeV/c^{2}");}
    else if(Peak == 2){Lpeak = new TLatex(1.0, 0.2, "0.70 < M_{clcl} < 0.85 GeV/c^{2}");}
    else if(Peak == 3){Lpeak = new TLatex(1.0, 0.2, "1.0 < M_{clcl} < 1.3 GeV/c^{2}");}

    TLegend *Legend1 = new TLegend (.15, .58, .5, .88);
    Legend1 -> SetFillStyle (0);
    Legend1 -> SetLineColor (0);
    Legend1 -> AddEntry (pt_asymfit1[a][0], "#epsilon_{sqrt}:a*cos(#varphi), x_{F} < -0.3");
    Legend1 -> AddEntry (pt_asymfit3[a][0], "#epsilon_{pol}:b+a*cos(#varphi), x_{F} < -0.3");
    Legend1 -> AddEntry (pt_asymfit5[a][0], "#epsilon_{pol}:#bar{b}+a*cos(#varphi), x_{F} < -0.3");
    Legend1 -> AddEntry (pt_asymfit1[a][1], "#epsilon_{sqrt}:a*cos(#varphi), x_{F} > 0.3");
    Legend1 -> AddEntry (pt_asymfit3[a][1], "#epsilon_{pol}:b+a*cos(#varphi), x_{F} > 0.3");
    Legend1 -> AddEntry (pt_asymfit5[a][1], "#epsilon_{pol}:#bar{b}+a*cos(#varphi), x_{F} > 0.3");
    //Legend1 -> AddEntry (pt_an1[a][0], "x_{F} < -0.3");
    //Legend1 -> AddEntry (pt_an1[a][1], "x_{F} > 0.3");
    
    TLine *zpt = new TLine (-0., 0., 5., 0.);
    if(Final == 0){
      char *asymxNameText = "./images_pt_rxy%d%d/asymp_peak%d_trig%d_nbins%d_arm%d.gif";
      sprintf (Hname, asymxNameText, RMIN, RMAX, Peak, Trig, nbins, a);
    }else{
      char *asymxNameText = "./images_pt/asymp_peak%d_trig%d_nbins%d_arm%d.gif";
      sprintf (Hname, asymxNameText, Peak, Trig, nbins, a);
    }
    TCanvas *Casymx = new TCanvas ("asym", "asym", 500, 500);
    //Casymx -> Divide (1, 2);
    //Casymx -> cd(1);
    //pt_asymfit1[a][0] -> Draw();
    //pt_asymfit3[a][0] -> Draw("SAME");
    //pt_asymfit5[a][0] -> Draw("SAME");
    //pt_asymfit1[a][1] -> Draw("SAME");
    //pt_asymfit3[a][1] -> Draw("SAME");
    //pt_asymfit5[a][1] -> Draw("SAME");
    //Legend1 -> Draw ("SAME");   
    //zpt -> Draw();
    Casymx -> cd();
    //pt_an5[a][0] -> Draw();
    //pt_an5[a][1] -> Draw("same");
    pt_an1[a][0] -> Draw();
    pt_an1[a][1] -> Draw("same");
    Lpeak -> Draw();
    zpt -> Draw();
    ...
    if(Trig == 0){
      if(nbins == 1){
	Lpt[0][neg] = new TLatex (-2.8, .08, "1.0 < p_{T} < 2.0, x_{F} < -0.3");
	Lpt[0][pos] = new TLatex (-2.8, .08, "1.0 < p_{T} < 2.0, x_{F} > 0.3");
      }
      else{
	Lpt[0][neg] = new TLatex (-2.8, .08, "1.0 < p_{T} < 1.5, x_{F} < -0.3");
        Lpt[1][neg] = new TLatex (-2.8, .08, "1.5 < p_{T} < 2.0, x_{F} < -0.3");
	Lpt[0][pos] = new TLatex (-2.8, .08, "1.0 < p_{T} < 1.5, x_{F} > 0.3");
        Lpt[1][pos] = new TLatex (-2.8, .08, "1.5 < p_{T} < 2.0, x_{F} > 0.3");

      }
    }
    if(Trig == 1){
      if(nbins == 1){
	Lpt[0][neg] = new TLatex (-2.8, .08, "p_{T} > 2.0, x_{F} < -0.3");
	Lpt[0][pos] = new TLatex (-2.8, .08, "p_{T} > 2.0, x_{F} > 0.3");
      }
      else{
	Lpt[0][neg] = new TLatex (-2.8, .08, "2.0 < p_{T} < 2.5, x_{F} < -0.3");
        Lpt[1][neg] = new TLatex (-2.8, .08, "2.5 < p_{T} < 3.0, x_{F} < -0.3");
        Lpt[2][neg] = new TLatex (-2.8, .08, "3.0 < p_{T} < 3.5, x_{F} < -0.3");
        Lpt[3][neg] = new TLatex (-2.8, .16, "3.5 < p_{T} < 4.0, x_{F} < -0.3");
        Lpt[4][neg] = new TLatex (-2.8, .16, "4.0 < p_{T} < 4.5, x_{F} < -0.3");
	Lpt[0][pos] = new TLatex (-2.8, .08, "2.0 < p_{T} < 2.5, x_{F} > 0.3");
        Lpt[1][pos] = new TLatex (-2.8, .08, "2.5 < p_{T} < 3.0, x_{F} > 0.3");
        Lpt[2][pos] = new TLatex (-2.8, .08, "3.0 < p_{T} < 3.5, x_{F} > 0.3");
        Lpt[3][pos] = new TLatex (-2.8, .16, "3.5 < p_{T} < 4.0, x_{F} > 0.3");
        Lpt[4][pos] = new TLatex (-2.8, .16, "4.0 < p_{T} < 4.5, x_{F} > 0.3");

      }
    }

    //    for (int k = 0; k < 3, k++){Lpt[k] -> SetTextSize (0.07);}
    TLine *phi_zero1 = new TLine (-PI, 0., 0., 0.);
    TLine *phi_zero2 = new TLine (-PI, 0., PI, 0.);

    TH1D *tasym1[2][nbins][2];
    TH1D *tasym[2][nbins][2];
    TH1D *tasym3[2][nbins][2];
    TH1D *tasym4[2][nbins][2];
    TH1D *tasym5[2][nbins][2];

    if(Final == 0){
      char *asymsNameText = "./images_pt_rxy%d%d/asyms_peak%d_trig%d_nbins%d_arm%d.gif";
      sprintf (Hname, asymsNameText, RMIN, RMAX, Peak, Trig, nbins, a);
    }else{
      char *asymsNameText = "./images_pt/asyms_peak%d_trig%d_nbins%d_arm%d.gif";
    ...
    Casym4 -> SaveAs (Hname);
    if(Final == 0){
      char *asymqNameText = "./images_pt_rxy%d%d/asymq_peak%d_trig%d_nbins%d_arm%d.gif";
      sprintf (Hname, asymqNameText, RMIN, RMAX, Peak, Trig, nbins, a);
    }else{
      char *asymqNameText = "./images_pt/asymq_peak%d_trig%d_nbins%d_arm%d.gif";
    ...
    Legend2 -> AddEntry (pt_asymfit1[a][b], "#epsilon_{sqrt}:a*cos(#varphi)");
    Legend2 -> AddEntry (pt_asymfit2[a][b], "#epsilon_{sqrt}:a*cos(#varphi+#varphi_{0})");
    Legend2 -> AddEntry (pt_asymfit3[a][b], "#epsilon_{pol}:b+a*cos(#varphi)");
    Legend2 -> AddEntry (pt_asymfit4[a][b], "#epsilon_{pol}:b+a*cos(#varphi+#varphi_{0})");

    if(Final == 0){
      char *fitsNameText = "./images_pt_rxy%d%d/fitsummary_peak%d_trig%d_nbins%d_arm%d.gif";
      sprintf (Hname, fitsNameText, RMIN, RMAX, Peak, Trig, nbins, a);
    }else{
      char *fitsNameText = "./images_pt/fitsummary_peak%d_trig%d_nbins%d_arm%d.gif";
    ...
    Cfits -> cd (1);
    pt_asymfit1[a] -> Draw();
    pt_asymfit2[a] -> Draw("same");
    pt_asymfit3[a] -> Draw("same");
    pt_asymfit4[a] -> Draw("same");
    Legend2 -> Draw ("same");
    zpt -> Draw();
   
    Cfits -> cd (3);
    pt_rellfit3[a] -> Draw();
    pt_rellfit4[a] -> Draw ("same");
    RLine[0] -> Draw();
    RLine[1] -> Draw();
 ...
 */   
  ...
  for (int k = 0; k < nbins; k++) {
    ...
    int kmin = ptTitleMin * 100 + 1;
    int kmax = ptTitleMax * 100 + 1;
    float meanxf;
    float rmspt;
    float meanpt;
    float meaneta;
    for(int x = kmin; x < kmax; x++)
      for(int m = pi0_left; m <= pi0_right; m++){
        //cout << "blah x m, value " << x << " " << m << endl;
        yptmass3 -> SetBinContent(m, x, yptmass -> GetBinContent(m, x));
        for(int p = 0; p < 100; p++){
          yptmassxf3 -> SetBinContent(m, x, p, yptmassxf -> GetBinContent(m, x, p));
        }
      }
    tpt3 = yptmass3 -> ProjectionY();
    meanpt = tpt3 -> GetMean();
    txf3 = yptmassxf3 -> ProjectionZ();
    meanxf = txf3 -> GetMean();
    cout << "meanpt, meanxf " << meanpt << " " << meanxf << endl;
    pt_mean -> SetBinContent(0.5 + (binjump + k) , meanpt);
    xf_mean -> SetBinContent(binjump + k , meanxf);
    //exit(1);
    tpt3 -> Reset();
    tpt3 -> Reset();
    yptmass3 -> Reset();
  ...
  float meaneta = yeta -> GetMean();
  cout << "The mean of pseudo-rapidity is " << meaneta << endl;

  //Write out to OutFile!!
  if(Final == 0){
    char *outNameText = "./results/asym_peak%d_trig%d_nbins%d_rxy%d%d.root";
    sprintf (outName, outNameText, Peak, Trig, nbins, RMIN, RMAX);
  }else{
    char *outNameText = "./results/asym_peak%d_trig%d_nbins%d.root";
  ...
  TFile *outFile = new TFile (outName, "RECREATE");
	...
	pt_asymfit1[a][b] -> Write();

	pt_asymfit2[a][b] -> Write();
	pt_phi0fit2[a][b] -> Write();

	pt_rellfit3[a][b] -> Write();
	pt_asymfit3[a][b] -> Write();

	pt_rellfit4[a][b] -> Write();
	pt_asymfit4[a][b] -> Write();
	pt_phi0fit4[a][b] -> Write();

	pt_asymfit5[a][b] -> Write();
