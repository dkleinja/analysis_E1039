...
void bunch_day(const int Trig = 0, const int Roadset = 67, const int NBins = 1)
{
  ...
  for(int a = 0; a < 2; a++){
    ...
    sprintf(Tname, "#chi^{2}/(NDF-1) for #LT1 Day#GT spin change");
	...
	if(NBins == 1){xfTitleMin = 0.2, xfTitleMax = 0.7;}
      ...
      sprintf(Tname, "A_{N} for #LT1 Day#GT spin change", Roadset, xfTitleMin, xfTitleMax, inArm);
      ...
      sprintf(Tname, "#sigma_{A_{N}} for #LT1 Day#GT spin change", Roadset, xfTitleMin, xfTitleMax, inArm);
      ...
      sprintf(Hname, "anval5_target%d_xf%d", a, k);
      sprintf(Tname, "Pol A_{N} 1 Day Shuff. for Roadset %d, %1.1f < x_{F} < %1.1f for %s", Roadset, xfTitleMin, xfTitleMax, inArm);
  ...
  sprintf(inName, "./results/asym2_roadset%d_nbins%d_byday0.root", Roadset, nbins);
  sprintf(inName, "./results_tighter/asym2_roadset%d_nbins%d_byday0.root", Roadset, nbins);
  ...
  for (int i = 1; i < NFILES; i++){
    ...
    //load the inFile   
    sprintf(inName, "./results/asym2_roadset%d_nbins%d_byday%d.root", Roadset, nbins, i);
    sprintf(inName, "./results_tighter/asym2_roadset%d_nbins%d_byday%d.root", Roadset, nbins, i);
    //printf(inName);
