void drawdp(const int spot = 5)
{
  ...
  sprintf(Fname, "./nDST/BeamAngle_tgt2.root");
  ...
  Double_t gpx_x[nbins] = {-130, -37.5, -12.5, 12.5, 37.5, 62.5, 87.5, 112.5};
  Double_t gpx_y[nbins];
  TGraph *Gpx = new TGraph();
  Gpx -> SetMarkerStyle(21);
  Gpx -> SetFillColor(0);
  Double_t gpy_x[nbins] = {-130, -37.5, -12.5, 12.5, 37.5, 62.5, 87.5, 112.5};
  Double_t gpy_y[nbins];
  TGraph *Gpy = new TGraph();
  Gpy -> SetMarkerStyle(21);
  Gpy -> SetMarkerColor(2);
  Gpy -> SetFillColor(0);
  Double_t gpz_x[nbins] = {-130, -37.5, -12.5, 12.5, 37.5, 62.5, 87.5, 112.5};
  Double_t gpz_y[nbins];
  ...
  TCanvas *cx8 = new TCanvas("cx8","cx8",2400,800);
  cx8 -> Divide(4,2);
  for(int i = 0; i < nbins; i++){
    cx8 -> cd(i+1);
    Hpx1[i] -> Draw();
  ...
  TCanvas *cy8 = new TCanvas("cy8","cy8",2400,800);
  cy8 -> Divide(4,2);
  for(int i = 0; i < nbins; i++){
    cy8 -> cd(i+1);
    Hpy1[i] -> Draw();
  ...
  TCanvas *cz8 = new TCanvas("cz8","cz8",2400,800);
  cz8 -> Divide(4,2);
  for(int i = 0; i < nbins; i++){
    cz8 -> cd(i+1);
    Hpz1[i] -> Draw();
