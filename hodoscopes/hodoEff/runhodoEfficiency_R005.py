...
#for run in range(0, 2822):
	...
	#dccp_file = "cp %s ./" % (flines[run])
        #print dccp_file
        hodostatement = "./hodoEfficiency ./roadset67DSTs/track_%s_r1.5.0.root ./root_files/R005files_track1_%s.root > ./logs/R005files_track1_%s.log 2>&1" % (rlines[run], rlines[run], rlines[run]) 
        #hodostatement = "./hodoEfficiency track_%s_r1.6.0.root ./root_files/R005files_nim1_%s.root" % (rlines[run], rlines[run]) 
