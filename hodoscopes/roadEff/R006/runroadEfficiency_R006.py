...
#for run in range(0, 2822):
	...
	dccp_nim1 = "dccp %s ./" % (nlines[run])
        print dccp_nim1
        roadstatement = "./roadEfficiency track_%s_r1.6.0.root ./root_files/R006files_track1_%s.root > ./logs/R006files_track1_%s.log 2>&1" % (rlines[run], rlines[run], rlines[run]) 
        #roadstatement = "./roadEfficiency track_%s_r1.6.0.root ./root_files/R006files_nim1_%s.root" % (rlines[run], rlines[run]) 
